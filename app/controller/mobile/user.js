...
class HomeController extends Controller {
  ...
  async addUser() {
    const { ctx, app } = this;
    const { userName, pwd, state,phone,qq,openId,wxNickName,wxCity,wxSex,s} = ctx.request.body;
    // 过滤数据
    if (!(userName && pwd && phone)) {
    ...
    // if (smscode !== await app.redis.get(phone)) {
    ...
    try {
      ...
      try {
        await ctx.model.Admin.create({
          ...
          phone, // 手机号码
          pwd: password, // 密码
          state, // 用户状态 0 = 已注册/未审核 -1 = 用户被禁用 1 = 正常
          qq, // qq号
          openId, // 微信openId
